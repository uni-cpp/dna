...
constexpr std::byte
complement_packed( std::byte packed )
{
    return packed ^ static_cast< std::byte >( 0 );
}

constexpr base
complement( enum base base )
{
    return static_cast< enum base >( complement_packed( static_cast< std::byte >( base ) ) & static_cast< std::byte >( 0x03 ) );
}

constexpr std::byte
pack( base first, base second, base third, base fourth )
{
    return ( static_cast< std::byte >( first ) << 6 ) | ( static_cast< std::byte >( second ) << 4 ) | ( static_cast< std::byte >( third ) << 2 )
...
constexpr packed_bases
unpack( std::byte b )
{
    return packed_bases( { static_cast< dna::base >( ( b >> 6 ) & std::byte{ 0x3 } ),
...
inline std::ostream&
operator<<( std::ostream& os, base v )
{
    switch( v )
    {
        case base::adenine:
